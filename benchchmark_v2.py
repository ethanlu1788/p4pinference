...
class PerformanceMonitor:
    """A helper class to track CPU, RAM, FPS, temperature, and power usage over time."""
    def __init__(self):
...
def benchmark_video_with_detailed_logging(model_name, model_infer_func,
    ...
    print(f"Saved {model_name} results. Peak temp {peak_temp:.1f}°C, Avg power {avg_power:.2f}W")

# --- Model loaders & inference functions ---
import tflite_runtime.interpreter as tflite
def load_tflite_model(path):
    it = tflite.Interpreter(model_path=path); it.allocate_tensors(); return it
def infer_tflite_model(it, x):
    id0 = it.get_input_details()[0]['index']
    it.set_tensor(id0,x); it.invoke()
...
def load_onnx_model(p):
    s=ort.InferenceSession(p); return s, s.get_inputs()[0].name
